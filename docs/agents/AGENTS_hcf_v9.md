...
#### Dynamic path detection (works in any git repo)
...
### Step 0.1: Cold-Start Detection & Knowledge Base Bootstrap

```bash
if [ ! -s "${AGENTS_ROOT}/context/company/release-style.md" ] || \
   [ ! -s "${AGENTS_ROOT}/context/RELEASE-KNOWLEDGE.md" ] || \
   [ ! -s "${AGENTS_ROOT}/context/skills/hotfix-skills.json" ]; then
    echo "=== COLD-START DETECTED ==="
    echo "Knowledge base not found. Running one-time bootstrap from live git history..."
    bash "${AGENTS_ROOT}/bootstrap-knowledge-base.sh" --auto > "${AGENTS_ROOT}/bootstrap.log" 2>&1

    if [ -s "${AGENTS_ROOT}/context/company/release-style.md" ] && \
       [ -s "${AGENTS_ROOT}/context/RELEASE-KNOWLEDGE.md" ] && \
       [ -s "${AGENTS_ROOT}/context/skills/hotfix-skills.json" ]; then
        echo "✅ Cold-start bootstrap completed successfully."
    else
        echo "❌ Bootstrap failed. See ${AGENTS_ROOT}/bootstrap.log"
        cat "${AGENTS_ROOT}/bootstrap.log"
        # Escalate with structured lettered options per escalation contract
    fi
fi
```

### Step 0.5: Load Adaptive Company Knowledge (mandatory)

```bash
cat "${AGENTS_ROOT}/context/RELEASE-KNOWLEDGE.md"
...
### Step 9.5: Reflection & Skill Update (Learner Loop)
