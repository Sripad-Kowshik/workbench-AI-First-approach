...
The CHF agent does not call peer agents directly. It emits structured handoff envelopes to the orchestrator. The orchestrator routes work to the diffing agent, packaging agent, and evaluator.

Required handoff envelope fields:
- task_id
- operation
- base_release
- target_release
...
### Step 2.5: Emit diff request
Create a structured diff request envelope for the orchestrator so the diffing agent can determine the effective change set and packaging inputs.
